...
import { deleteData, getDataById, updateData } from "../../logic/users";
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        const token = verifyToken(req);
        if (!token) throw { message: "Unauthorized", statusCode: 401 };
        if (!hasPermission(token, "user:update")) throw { message: "Forbidden", statusCode: 403 };
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        const token = verifyToken(req);
        if (!token) throw { message: "Unauthorized", statusCode: 401 };
        if (!hasPermission(token, "user:delete")) throw { message: "Forbidden", statusCode: 403 };
