...
import { apiPath, pageRoute, columnFormats as baseColumns, breadcrumb, title, filterFields } from "./page.config"
...
import roleDefinition from "./access-definition"
...
export default function RolePage() {
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            // beri delay 1,5 detik baru refresh
            setTimeout(() => {
                window.location.reload(); // memaksa reload data
            }, 1500)
        ...
        <>
            <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                    ...
                    {hasPermission(user, `${roleDefinition.id}:read`) && (
                        <DropdownMenuItem onClick={() => router.push(pageRoute.details(id))}>
                            View Data
                        </DropdownMenuItem>
                    )}
                    {hasPermission(user, `${roleDefinition.id}:update`) && (
                        <DropdownMenuItem onClick={() => router.push(pageRoute.edit(id))}>
                            Edit Data
                        </DropdownMenuItem>
                    )}
                    {hasPermission(user, `${roleDefinition.id}:delete`) && (
    ...
    return (
        <SidebarProvider>
            ...
            <DynamicPage
                ...
                apiPath={`${apiPath}/api`}
                pageRoute={pageRoute}
                showAddAction={hasPermission(user, `${roleDefinition.id}:create`)}
